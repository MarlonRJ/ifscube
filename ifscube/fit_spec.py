...
# initialization
#plt.ion()
#path_cfg = os.path.realpath(os.path.join(os.path.dirname(__file__), '..','ifscube' ,'examples', 'halpha_gauss_diff.cfg'))
...
#coord = np.array([[184,214],[192,203]],dtype='int64')
...
image_path =os.path.realpath(os.path.join(os.path.dirname(__file__), '..','ifscube' ,'images'))
#print(image_path)
for i in range(4):
    for j in range(3):
...
for row in range(1):
    ...
    path = os.path.realpath(os.path.join(os.path.dirname(__file__), '..','ifscube','spec_fits', name_fits))
    ...
    #print('cheguei Aqui')
    #plt.show() 
    plt.savefig(os.path.join(image_path,plot_fits),dpi = 300)
    #print('sai aqui')
    plt.close()
